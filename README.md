...
## Как пользоваться?
1. *Скачать скрипты* `ReportMaker.py` и `CreateForumTopics.py` в какой-нибудь каталог на компьютере
2. *Скачать драйвер* своего браузера и настроить интерпретатор Python (см. ниже)
3. *В начале недели, перед занятиями*, запустить скрипт `CreateForumTopics.py`
4. *Видеозаписи* проведённых занятий должны именоваться последовательно Video1 Video2 ... Video9 (одно видео - одна пара). Желательно, чтобы они находились в отдельной папке на диске
5. *Cоздать текстовый файл* `settings.txt` в каталоге скрипта (см. ниже)
...
Скрипт формирования отчёта запускается скрипт из командной строки 
```bash
...
* Сделана проверка имени видео файла по шаблону
