...
Используем скрипт `getWeekData.py` для формирования файла с данными обо всех занятиях престоящей недели.
...
Скрипт `CreateForumTopics.py` генерирует топики в разделе Сервисы-Форум для отметки и верификации присутствия студентов 
на занятиях.
Команда 
```bash
...
```
либо кликая `ReportMaker.bat`. Если отчёт заполняется за другой день, надо передать дату через командную строку `python ReportMaker.py дд.мм.гг` или `ReportMaker.bat дд.мм.гг` (заполнить можно только отчёты текущей недели)
3. Идём *пить чай*
